...
/**
 * Normaliza un teléfono mexicano a formato wa.me.
 * - Quita todo lo que no sea dígito.
 * - Si tiene 10 dígitos (formato MX), antepone 52.
 * - Si ya empieza con 52, lo deja.
 * - Si tiene menos/más dígitos, lo deja como está (el link wa.me igual abre).
 */
export function toWhatsAppUrl(phone = '') {
  const digits = String(phone).replace(/\D/g, '');
  if (!digits) return '';
  if (digits.length === 10) return `https://wa.me/52${digits}`;
  if (digits.startsWith('52') && digits.length >= 12) return `https://wa.me/${digits}`;
...
export const ICON = {
  ...
  check: '<svg viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5 sm:h-6 sm:w-6" aria-hidden="true"><circle cx="12" cy="12" r="6"/></svg>',
